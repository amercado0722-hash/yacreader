...
public:
    ...
    // dot (e.g. ".cbz"); pass an empty string for none. When numberPadding is
    // greater than zero the {number} token is zero-padded to that width.
    ...
    // Replaces characters that are invalid in a path segment and trims it.
    static QString sanitizeSegment(QString segment);

    // Zero-pads the leading digits of an issue number to at least "width"
    // characters (e.g. "1" -> "01"). Non-numeric prefixes are left untouched.
